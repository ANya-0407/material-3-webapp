//EyesFollowingListコンポーネントを作成

import EyesFollowingProfileCard from 'src/components/parts/EyesFollowingProfileCard';
import { EyesFollowProfileApiProps, EyesFollowProfileProps, convertEyesFollowProfileProps } from "src/types";
import styles from 'src/styles/EyesFollowingList.module.css';
...
const EyesFollowingList: React.FC = () => {
    // ルーター
    ...
    return (
        <>
            {noMoreProfiles && centerProfiles.length === 0 ? (
                ...
                <div className={styles.eyes_following_list_wrapper} ref={centerColumnRef}>
                    {centerProfiles.map((profile) => (
                        <EyesFollowingProfileCard key={profile.accountId} profile={profile} />
                    ))}
...
export default EyesFollowingList;
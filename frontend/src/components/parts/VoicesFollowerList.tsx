//VoicesFollowerListコンポーネントを作成

import VoicesFollowerProfileCard from 'src/components/parts/VoicesFollowerProfileCard';
import { VoicesFollowProfileApiProps, VoicesFollowProfileProps, convertVoicesFollowProfileProps } from "src/types";
import styles from 'src/styles/VoicesFollowerList.module.css';
import { useRouter } from "next/router";
import React, { useState, useRef, useEffect, useCallback } from "react";

// サーバーの代わりとなるサンプルデータ
const sampleProfiles: (VoicesFollowProfileApiProps | null)[] = [
    {
        account_id: "1",
        account_name: "Amane1",
        account_icon: "/samples/image1.jpg",
        account_explanation: "これはサンプルです。意味はありません。",
        is_official: true,
        is_mutual: true
    },
    {
        ...
        account_explanation: "『Neverness to Everness』（NTE）はHotta Studioが自主開発した超現実アーバンオープンワールドRPGです。",
        is_official: true,
        is_mutual: false
    },
    {
...
const VoicesFollowerList: React.FC = () => {
    // ルーター
    const router = useRouter();

    // プロファイルの状態管理（センターカラムのみ使用）
    const [centerProfiles, setCenterProfiles] = useState<VoicesFollowProfileProps[]>([]);
    ...
    const serverDataRef = useRef<(VoicesFollowProfileApiProps | null)[]>([...sampleProfiles]);

    // サーバーからプロファイルを1件ずつ取得する関数
    const fetchProfiles = useCallback(async (): Promise<VoicesFollowProfileProps | null> => {
        ...
        return convertVoicesFollowProfileProps(profile);
    }, [noMoreProfiles]);
        ...
        <>
            {noMoreProfiles && centerProfiles.length === 0 ? (
                <div className={styles.error_background}>
                    ...
                    <button onClick={goVoicesHome}>
                        <div></div>
                        <span>いざステージへ！</span>
                    </button>
                </div>
            ) : (
                <div className={styles.voices_follower_list_wrapper} ref={centerColumnRef}>
                    {centerProfiles.map((profile) => (
                        <VoicesFollowerProfileCard key={profile.accountId} profile={profile} />
                    ))}
...
export default VoicesFollowerList;
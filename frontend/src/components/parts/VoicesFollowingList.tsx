//VoicesFollowingListコンポーネントを作成

import VoicesFollowingProfileCard from 'src/components/parts/VoicesFollowingProfileCard';
import { VoicesFollowProfileApiProps, VoicesFollowProfileProps, convertVoicesFollowProfileProps } from "src/types";
import styles from 'src/styles/VoicesFollowingList.module.css';
...
const VoicesFollowingList: React.FC = () => {
    // ルーター
    ...
    return (
        <>
            {noMoreProfiles && centerProfiles.length === 0 ? (
                ...
                <div className={styles.voices_following_list_wrapper} ref={centerColumnRef}>
                    {centerProfiles.map((profile) => (
                        <VoicesFollowingProfileCard key={profile.accountId} profile={profile} />
                    ))}
...
export default VoicesFollowingList;